...
# ---------------------------------------------------------------------------
# Model preset. The default remains the validated 0xSero rank-sliced build.
# The TurboDerp checkpoint lives on the 2.77bpw branch, not on main.
...
case "$MODEL_VARIANT" in
  default|0xsero)
    ...
    MODE_DEFAULT=dspark
    LOAD_FORMAT_DEFAULT=instanttensor
    B12X_WO_PROJECTION_DEFAULT=1
    RUNTIME_IMAGE_DEFAULT="ghcr.io/0xsero/deepseek-v4-flash-0731-spark-sparkinfer@sha256:2e077489a83a0360952828051fe7f7a32c1801e5ce8436d85f7267583d614ff4"
    RUNTIME_PULL_POLICY=always
    RUNTIME_BUILD_BLOCK=""
    ;;
  turboderp|turboderp-2.77bpw)
    ...
    # The published MTP weights are a separate 7.6 GB file. The image's
    # compact-draft builder is specific to the default REAP checkpoint, so
    # serve this variant without speculative decoding unless explicitly
    # supported in a future runtime.
    MODE_DEFAULT=mtp0
    LOAD_FORMAT_DEFAULT=safetensors
    B12X_WO_PROJECTION_DEFAULT=0
    RUNTIME_IMAGE_DEFAULT=deepseek-v4-flash-spark-turboderp-exl3:704aefd-sm121
    RUNTIME_PULL_POLICY=build
    RUNTIME_BUILD_BLOCK=$'    build:\n      context: .\n      dockerfile: Dockerfile.turboderp'
    ;;
  *)
    echo "ERROR: MODEL_VARIANT must be default or turboderp-2.77bpw (got '$MODEL_VARIANT')" >&2
    exit 1
    ;;
esac
...
MODE="${MODE:-$MODE_DEFAULT}"          # default model: K5 DSpark; TurboDerp: target-only
LOAD_FORMAT="${LOAD_FORMAT:-$LOAD_FORMAT_DEFAULT}"
VLLM_USE_B12X_WO_PROJECTION="${VLLM_USE_B12X_WO_PROJECTION:-$B12X_WO_PROJECTION_DEFAULT}"
...
generate_compose() {
  ...
  # Generic EXL3 uses the ExLlamaV3 parity kernels. They autotune on first use
  # and cannot run inside CUDA graph capture.
  if [ "$MODEL_LAYOUT" = "hf" ]; then
    EXTRA_VLLM_ARGS="${EXTRA_VLLM_ARGS} --enforce-eager"
  fi
  EXTRA_VLLM_ARGS="${EXTRA_VLLM_ARGS# }"   # strip leading space
...
# Selected weights:
#   https://huggingface.co/${MODEL_REPO}/tree/${MODEL_REVISION}
...
# REMOTE_HOST is set). Rank-sliced weights are coalesced into ./data/tp1;
# ready-to-serve Hugging Face checkpoints are loaded from the snapshot.
name: deepseek-v4-flash-spark
services:
  deepseek-v4-flash:
    image: ${RUNTIME_IMAGE}
    pull_policy: ${RUNTIME_PULL_POLICY}
${RUNTIME_BUILD_BLOCK}
    ...
    environment:
      ...
      # path /hf-cache/...), so the download is not duplicated under ./data
      # for ready-to-serve checkpoints. Local mode uses ./hf-hub.
...
YAML
  ...
  echo "  model layout  : $MODEL_LAYOUT"
...
cmd_start() {
  ...
  echo ">> First boot is intentionally long: prepares the runtime and downloads"
  if [ "$MODEL_LAYOUT" = "rank-sliced" ]; then
    echo "   ~107 GB of weights, coalesces TP4->TP1, and builds the K64 draft."
  else
    echo "   the $MODEL_REVISION branch of $MODEL_REPO into the local HF cache."
  fi
  echo "   Waiting up to ${STARTUP_WAIT}s for"
...
case "${1:-start}" in
  ...
  pull)
    generate_compose
    shift
    if [ "$RUNTIME_PULL_POLICY" = "build" ]; then
