...
```bash
./start.sh      # default: 0xSero 3.0bpw, deep-context NVFP4, DSpark
./start.sh --no-wait

# Optional full 256-expert TurboDerp checkpoint. This selects the 2.77bpw
# branch explicitly; Hugging Face's main branch does not contain the weights.
MODEL_VARIANT=turboderp-2.77bpw ./start.sh
```

The default first boot pulls the image, downloads ~107 GB into `./hf-hub`, coalesces TP4→TP1, builds the K64 draft, and captures CUDA graphs. The TurboDerp preset downloads about 108 GB directly from the `2.77bpw` branch and builds a derived runtime containing the pinned SM121 ExLlamaV3 extension plus the DeepSeek V4 adapters needed by its fully quantized HF layout. It uses the standard safetensors loader and serves target-only with `MODE=mtp0` and eager execution because generic EXL3 kernels cannot use CUDA graphs. The server is marked `healthy` only when the OpenAI-compatible endpoint responds.

The 384k TurboDerp profile was boot-tested on a GB10 GX10: model residency was 94.2 GiB, the runtime allocated 13.68 GiB of KV cache (737,514 tokens, 1.92× concurrency at 384k), and a chat completion succeeded. Keep the host otherwise idle at launch; unified memory headroom is tight.
...
download+coalesce+verify standalone (also fully local). Once
  ...
  download/coalesce step entirely, so a network-free runtime follows. For the
  TurboDerp branch, run `MODEL_VARIANT=turboderp-2.77bpw ./download.sh`; its
  completion marker stays in that branch's HF snapshot directory.
...
| `Dockerfile.turboderp` | Derived runtime used only by the TurboDerp preset; builds the pinned SM121 ExLlamaV3 EXL3 extension |
...
|---|---|---|
| `MODEL_VARIANT` | `default` | `turboderp-2.77bpw` selects `turboderp/DeepSeek-V4-Flash-0731-exl3` at revision/branch `2.77bpw`; aliases: `turboderp`, `0xsero` |
| `MODEL_REPO` / `MODEL_REVISION` | preset-dependent | Low-level overrides. The TurboDerp weights are not on `main`, so keep revision `2.77bpw` unless intentionally selecting another branch. |
...
| `MODE` | `dspark` | fixed K5 DSpark draft for the default model. The TurboDerp preset defaults to `mtp0` because its separate MTP file is not compatible with the REAP-specific compact-draft builder. |
| `LOAD_FORMAT` | preset-dependent | `instanttensor` for the unchanged default preset; `safetensors` for TurboDerp's standard HF checkpoint. |
